...
- A non-smooth RHS pushes the other way, but it does NOT simply reverse the rule. An implicit method solves a nonlinear system at every step, and across a discontinuity that solve cannot converge — the integration then fails for ANY `max_steps`, so raising it does not help. The sources are `sign`, `abs`, `floor`, `clip`, or a state-dependent `where` (dry friction, contact, saturation, hysteresis, on/off control). But that cost is LOCAL, paid only at the switching instants, so it scales with how often the trajectory crosses one — whereas stiffness constrains every step over the whole interval. The two are therefore weighed by magnitude, not resolved by a gate: a stiff system with rare one-way switches still wants an implicit solver. This digest does not own that decision — see R1 in `lib/LLM/reference/tuning_rules.md`. Either failure scores `error_loss`, so the cost is not just runtime: whole regions of parameter space become invisible to the optimizer.
